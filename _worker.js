...
let userID = 'fd57abd1-91eb-4323-aa5e-f1abf7c94d95';//需自定义uuid
	...
	userIDArray.forEach((userID) => {
		...
		if (!hostName.includes('pages.dev')) {
			...
			portArray_http.forEach((port) => {
				const commonUrlPart_http = `:${port}?encryption=none&security=none&fp=chrome&type=ws&host=${hostName}&path=%2F%3Fed%3D2048#${hostName}-HTTP-${port}`;
		...
		portArray_https.forEach((port) => {
			const commonUrlPart_https = `:${port}?encryption=none&security=tls&sni=${hostName}&fp=chrome&type=ws&host=${hostName}&path=%2F%3Fed%3D2048#${hostName}-HTTPS-${port}`;
